...
def create_boundary_map(
    ...
    """
    Create a boundary map highlighting Σ3 and Σ9 twin boundaries using vectorized operations.
    ...
    Returns:
        (H, W) boundary map array (0=interior, 1=Σ3, 2=Σ9, 3=both)
    """
    H, W = grid_shape

    print(f"Checking Σ3 and Σ9 boundaries...")
    ...
    # Initialize boundary maps (separate for Σ3, Σ9, and general boundaries)
    ...
    # Compute minimum deviation from Σ3 and Σ9 twins
    ...
    if n_general > 0:
        ...
        frac_both = 100 * general_and_both / n_general

        print(f"  Fraction that are Σ3: {frac_sigma3:.2f}%")
        print(f"  Fraction that are Σ9: {frac_sigma9:.2f}%")
        print(f"  Fraction that are both: {frac_both:.2f}%")

    return boundary_map, boundary_map_general, boundary_map_sigma3, boundary_map_sigma9


def visualize_boundary_map(boundary_map, boundary_map_general, output_dir):
    """
    Visualize and save the boundary map in publication-ready format.

    Args:
        boundary_map: (H, W) array with 0=interior, 1=Σ3, 2=Σ9, 3=both
    ...
    rgb_map[boundary_map == 3] = [1.0, 0.0, 1.0]
    ...
    # Clean title
    ax.set_title("Σ3 and Σ9 Boundary Map", fontsize=16, pad=10)
    ax.axis("off")
    ...
    output_file = output_dir / "figure_sigma3_sigma9_boundary_map.png"
    plt.savefig(output_file, dpi=DPI, bbox_inches="tight", facecolor="white")
    print(f"✓ Boundary map saved to: {output_file}")

    output_pdf = output_dir / "figure_sigma3_sigma9_boundary_map.pdf"
...
if __name__ == "__main__":
    ...
    if not data_file.exists():
        ...
        visualize_boundary_map(boundary_map, boundary_map_general, output_dir)
        ...
        if np.any(mask):
            ...
            if n_errors > 0:
                ...
                # Errors at CSL boundaries (either Σ3 or Σ9)
                errors_at_csl = np.sum(
                    indexing_errors
                    & ((boundary_map_sigma3 > 0) | (boundary_map_sigma9 > 0))
                )
                ...
                print(
                    f"  At CSL boundaries (Σ3 or Σ9): {errors_at_csl} ({frac_at_csl:.2f}%)"
                )
                print(
